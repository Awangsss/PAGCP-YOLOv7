...
def compress(model, dataloader, args):
    ...
    if os.path.exists(model_path):
        ...
        DG = DependencyGraph()
        DG.build_dependency(pruned_model, example_inputs=torch.randn(1, 3, imgsz[0], imgsz[1]))
...
def train(hyp, opt, device, tb_writer=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, freeze, compress_round = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank, opt.freeze, opt.round
    ...
    if not opt.pruned:
        ...
        with torch_distributed_zero_first(rank):
            pruned_model = compress(model, testloader if rank in [-1, 0] else None, opt)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pruned_model', type=str, default='', help='the path of the pruned model')
    parser.add_argument('--pruned_layers', type=int, default=0, help='pruned-layers')
    opt = parser.parse_args()
    # opt.part = [f'model.{i}.' for i in range(opt.pruned_layers)]
    opt.part = [f'model.{i}.' for i in range(23)] + [f'model.{i}.' for i in range(40, 55)]
